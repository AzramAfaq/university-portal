...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const attendanceRes = await client.query('SELECT * FROM attendance WHERE student_id = $1', [studentId]);
    await client.end();
    return NextResponse.json({ success: true, attendance: attendanceRes.rows });
  } catch (error) {
    await client.end();
